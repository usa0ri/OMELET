...
function [data_layers,data_edges] = make_data_layers(obj)
    ...
    data_met = nan(length(met_names),obj.model_data.num_smpl);
    data_pro = nan(length(rxn_names),obj.model_data.num_smpl);
    data_rna = nan(length(rxn_names),obj.model_data.num_smpl);
    data_met(dict_m.idx>0,:) = data_m';
    data_pro(dict_p.idx>0,:) = data_p';
    data_rna(dict_t.idx>0,:) = data_t';
    ...
    dict_m_eff = make_dict_omics(dict_omics,...
        met_names_eff,[]);
    data_m_eff = make_data_omics(data_omics,...
        dict_m_eff,[],[]);
    data_met_eff = nan(length(met_names_eff),obj.model_data.num_smpl);
    data_met_eff(dict_m_eff.idx>0,:) = data_m_eff';
