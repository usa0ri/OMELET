function makeFig2(obj,savedir_)
   ...
   name_omics = obj.omics_data.name_omics;
    ...
    pmat = cell(1,3);
    for i=1:length(dict_omics)
        plot_indv_bar_omics(data_omics{i},dict_omics{i},name_omics{i},obj.model_data,savedir);
        plot_bar_omics(data_omics{i},dict_omics{i},name_omics{i},obj.model_data,savedir);
        pmat{i} = ttest_omics(data_omics{i},dict_omics{i},name_omics{i},obj.model_data,savedir);
    end
    
end


function plot_bar_omics(data_now,dict_now,name_now,info,savedir)
...
bar_col = info.col;
bar_names = dict_now.val;

plot_errbars(bar_input,err_input,bar_col,bar_names,name_now,savedir);

end

function plot_indv_bar_omics(data_now,dict_now,name_now,info,savedir)
...
function p_mat = ttest_omics(data_now,dict_now,name_now,info,savedir)

var_names = dict_now.val;
...
%     fid = fopen([savedir '/ttest_' name_now '_' grp1 '_' grp2 '.txt'],'w');
%     for i=1:length(var_names)
%         p = p_list(i);
%         h1 = is_sig1(i);
%         h2 = is_sig2(i);
