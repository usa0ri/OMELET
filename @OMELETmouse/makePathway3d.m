...
%     fig1 = gcf;
%     ax1 = gca;
%     lib.set_minmargin(fig1,ax1);
%     hold on;
%     fig1 = plot_grid(fig1,ax1);
%     
%     xy_met_int = nan(length(met_names),2);
%     disp('Specify the coordinates of metabolites');
%     for m=1:length(met_names)
%         disp(['Specify the coordinates of ' met_names{m}]);
%         [x_,y_] = ginput(1);
%         x = round(x_,2);
%         y = round(y_,2);
...
%     end
%     
%     xy_met_eff = nan(length(met_names_eff),2);
%     for m=1:length(met_names_eff)
%         disp(['Specify the coordinates of ' met_names_eff{m}]);
%         [x_,y_] = ginput(1);
%         x = round(x_,2);
%         y = round(y_,2);
%         scatter(x,y,'k','filled');
...
%     end
%     
%     xy_met_ = [xy_met_int;xy_met_eff];
%     
%     xy_met = nan(size(xy_met_));
%     % flip
%     xy_met(:,1) = xy_met_(:,1)/100;
%     xy_met(:,2) = (350 - xy_met_(:,2))/100;
    
    %     keybord;
%     fig = figure();
%     scatter(xy_met(:,1),xy_met(:,2));
...
%         -0.15 1.25;...% OAA
%         ];
%     % OAA, Cit, Isocit, AKG, SucCoA, Suc, Fum, Mal
%     theta_int = 2*pi/7;
%     xy_met_TCA = [cos(-theta_int.*[0 1 2 2 3 3 4 5 6]);...
%         sin(-theta_int.*[0 1 2 2 3 3 4 5 6])]';
%     shift_val = [-0.15 1.25] - xy_met_TCA(end,:);
%     xy_met_TCA = xy_met_TCA + shift_val;
% 
%     % Glu
%     xy_Glu = [xy_met_TCA(3,1)+0.3,...
%         xy_met_TCA(3,2)-0.3];
% 
%     xy_met = [xy_met_; xy_met_TCA(1:end-1,:); xy_Glu];
%     num_met = length(xy_met);
% 
%     xy_met_eff = [0.5 4.5;...% ATP
%         0.75 4.5;...% ADP
%         0.5 4;...% NAD
%         0.75 4;...% NADH
%         0.5 3.5;...% GDP
