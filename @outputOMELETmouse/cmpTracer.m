...
%         v_tracer_nan(p,:) = v_tracer_tmp;
    ...
    [cor_now, p_now] = corrcoef(v_tracer(:),v_tmp(:),'rows','complete');
    title(['Pearson = ' num2str(round(cor_now(1,2),3)) ' (p='...
        num2str(sprintf('%.2e',p_now(1,2))) ')'],'FontSize',10);
    ...
    % remove glycogenolysis
    rm_flux = {'glycogenolysis',...
        'glycogenolysis_GNG',...
        'glycogenolysis_TCA',...
        'glycogenolysis_GNG_TCA'};
    idx_list = {[1 3 4 5 6],...
        [1 4 5 6],...
        [1 3 4 5],...
        [1 4 5]};
    
    for i=1:length(rm_flux)
        idx_v = idx_list{i};
        fig = figure('visible','off');
        fig.PaperUnits = 'inches';
        fig.PaperPositionMode = 'manual';
    ...
    %     v_tracer_nan = nan(size(v_tracer));
        for p=1:num_p
            v_tracer_tmp = v_tracer(p,idx_v);
    %         v_tracer_tmp(v_tracer_tmp==0) = nan;
    %         v_tracer_nan(p,:) = v_tracer_tmp;
            eb(1) = errorbar(v_tracer_tmp,mu_v(idx_v),...
                ci_data(p,idx_v),ci_data(p,idx_v),...
                'horizontal', 'LineStyle', 'none',...
                'Color',col_list(p,:),'LineWidth',1);
            eb(2) = errorbar(v_tracer_tmp,mu_v(idx_v),...
                mu_v(idx_v)-ci_v(1,idx_v),...
                ci_v(2,idx_v)-mu_v(idx_v),...
                'vertical', 'LineStyle', 'none',...
                'Color',col_list(p,:),'LineWidth',1);
            scatter(v_tracer_tmp,mu_v(idx_v),...
                'SizeData',40,...
                'MarkerFaceColor',col_list(p,:),...
                'MarkerEdgeColor',col_list(p,:));
        end
        v_tmp = repmat(mu_v(idx_v),num_p,1);
        v_tracer_tmp = v_tracer(:,idx_v);
        [cor_now, p_now] = corrcoef(v_tracer_tmp(:),v_tmp(:),'rows','complete');
        title(['Pearson = ' num2str(round(cor_now(1,2),3)) ' (p='...
            num2str(sprintf('%.2e',p_now(1,2))) ')'],'FontSize',10);
        xlabel('Fold changes in the previous studies');
        ylabel('Fold changes inferred by OMELET');
        set(findobj(gca,'type','axes'),'FontSize',10,'FontName','SansSerif');
        print(fig,'-painters',...
            [savedir '/scatter_fluxFC_no_' rm_flux{i} '_allGroup.pdf'],...
            '-dpdf','-bestfit');
        close all;
    end
...
%         v_tracer_tmp(v_tracer_tmp==0) = nan;
    ...
    print(fig,'-painters',...
        [savedir '/scatter_fluxFC_eachGroup.pdf'],...
        '-dpdf','-bestfit');
    close all;
...
%     ci_v = [std(v,1);std(v,1)];
    ...
    v_tmp = repmat(mu_v,num_p,1);
    [cor_now, p_now ] = corrcoef(v_tracer_nan(:),v_tmp(:),'rows','complete');
    title(['Pearson = ' num2str(round(cor_now(1,2),3)) ' (p='...
        num2str(sprintf('%e',p_now(1,2))) ')'],'FontSize',10);
