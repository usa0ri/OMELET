function makeRstanInput(obj,S_path,noise,savedir)
...
function D = load_sim_data(X,a,noise)

[dict_m,dict_p,dict_m_eff] = make_dict_sim(a,X);
[data_m,data_p,data_m_eff,a] = make_data_sim(a,noise,...
...
idx_noise = ismember([0.01 0.05 0.1],noise);
if any(idx_noise)
    a.rateOut('WT');
    for i=1:length(a.strain_mut_name)
        a.rateOut(a.strain_mut_name{i});
    end
end
...
D.flux_ref = mu_v_wt./mu_v_wt(1);
D.flux_smpl = mu_v_wt_smpl./mu_v_wt(1);
D.flux_mut_ref = mu_v_g./mu_v_wt(1);
D.flux_mut_smpl = mu_v_g_smpl./mu_v_wt(1);
...
function [data_m,data_p,data_m_eff,a] = make_data_sim(a,noise,...
    dict_m,dict_p,dict_m_eff)

idx_var = [a.struct.tbl_m;a.struct.tbl_p];
tbl_WT = a.tbl_mat.tbl_out;
tbl_mut_mat = a.tbl_mat_mut;
idx_noise = ismember([0.01 0.05 0.1],noise);
if any(idx_noise)
    tbl_WT{idx_var.idxSimbio,5:end} = a.tbl_mat.data_noise{idx_noise};
    for i=1:length(a.strain_mut_name)
        tbl_out_now = tbl_mut_mat{i}.tbl_out;
        tbl_out_now{idx_var.idxSimbio,5:end} = a.tbl_mat_mut{i}.data_noise{idx_noise};
        tbl_mut_mat{i}.tbl_out = tbl_out_now;
    end
    a.tbl_mat.tbl_out = tbl_WT;
    a.tbl_mat_mut = tbl_mut_mat;
end
